...
def givens(bit_string_1: str,
    ...
    ctrl_state = bit_string_1_reversed[:target] + bit_string_1_reversed[
        (target + 1):]  # This provides what type of control
    ctrl_state = ctrl_state[::-1]  # This reversal cancels some other reversal

    multiRX = RXGate(angle).control(
        num_ctrl_qubits=len(ctrls),
        ctrl_state=ctrl_state)  # This builds the multicontrol X gate function

    # obsolete:
    # multiX = XGate().control(num_ctrl_qubits = 1, ctrl_state=ctrl_val)

    Xcirc = QuantumCircuit(
        num_qubits
