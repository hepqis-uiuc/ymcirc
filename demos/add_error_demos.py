...
from lattice_tools.error import calculate_statistical_error
from lattice_tools.error import calculate_trotter_error
...
graph_with_stat_error = True
graph_with_trotter_error = True
...
n_trotter_steps_cases = [3, 5] # Make this a list that iterates from 1 to 3
...
if __name__ == "__main__":
    ...
    for sim_time in sim_times:
        for n_trotter_steps in n_trotter_steps_cases:
            ...
            stat_error = 0; trotter_error = 0
            if graph_with_stat_error:
                stat_error = calculate_statistical_error(df_job_results.loc[current_sim_idx, current_vacuum_state] / n_shots, n_shots)
            if graph_with_trotter_error:
                trotter_error = calculate_trotter_error(sim_time, n_trotter_steps, df_job_results.loc[current_sim_idx, current_vacuum_state] / n_shots)
            df_job_results.loc[current_sim_idx, "error"] = np.sqrt(stat_error**2 + trotter_error**2)
    ...
    for n_steps in n_trotter_steps_cases:
        ...
        extracted_data.plot(y = "vacuum_persistence_probability", yerr = "error", label = f"$N_T = {n_steps}$", ax=ax)
