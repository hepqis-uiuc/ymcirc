...
n_trotter_steps_cases = [2,3,4,5,6] # Make this a list that iterates from 1 to 3
...
if __name__ == "__main__":
    ...
    for sim_time in sim_times:
        for n_trotter_steps in n_trotter_steps_cases:
            ...
            if current_vacuum_state not in job_result[0].data.meas.get_counts().keys():
                ...
                df_job_results.loc[current_sim_idx, "vacuum_persistence_probability"] = 1-df_job_results.loc[current_sim_idx, current_vacuum_state] / n_shots
    ...
    for n_steps in n_trotter_steps_cases:
        ...
        coeffs = np.polyfit(extracted_data.index.values.astype(float), extracted_data["vacuum_persistence_probability"].to_numpy().astype(float), 2)
        models.append([n_steps, list(coeffs)])
        #plt.plot(extracted_data.index.values.astype(float), model(extracted_data.index.values.astype(float)), label=f'{n_steps} Fit')
        extracted_data.plot(y = "vacuum_persistence_probability", label = f"$N_T = {n_steps}$", ax=ax)
        print(models)
    with open('models.txt', 'w') as filehandle:
        json.dump(models, filehandle)
    plt.title(f'Trotter Error (Sqrt(1-|<UdagU>|^2)) ({n_plaquettes} plaquettes, mat. trunc = {mag_hamiltonian_matrix_element_threshold})')
    plt.xlabel('Time')
    plt.ylabel('Error')
    ...
    plt.legend()
    plt.show()
    fig.savefig(PLOTS_DIR / Path(f"{n_plaquettes}-plaquettes-in-d={dimensions}-irrep_trunc={trunc_string}-mat_elem_cut={mag_hamiltonian_matrix_element_threshold}-trotter-error.pdf"))

    #Uncomment to save all job counts to disk.
    print("Saving data to disk...")
    df_job_results.to_csv(SIM_RESULTS_DIR / Path(f"{n_plaquettes}-plaquettes-in-d={dimensions}-irrep_trunc={trunc_string}-mat_elem_cut={mag_hamiltonian_matrix_element_threshold}-trotter-error.csv"))
    print("Done. Goodbye!")
