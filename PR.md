...
This PR refactors ymcirc to support configurable F-order (half-link ordering convention) throughout the codebase. The refactor updates the JSON data loading pipeline to handle the new `.json.gz` format with metadata wrappers produced by pyclebsch, restructures plaquette state control links from a flat tuple to a nested per-vertex tuple format, threads an `forder` parameter through the lattice definition and encoder classes, and makes all control-link-dependent logic (consistency checks, duplicate pruning, circuit construction) F-order-aware rather than hard-coded to the default ordering. Along the way, the Hamiltonian data pipeline was generalized to support dict-valued matrix elements (keyed by plane and signature) for future nonperiodic/higher-dimensional lattice support, and a critical performance bottleneck in `ParsedLatticeResult` construction was identified and resolved—achieving a ~2,574x speedup for d=2 configurations by eliminating unnecessary deep copies and caching repr strings.

## History

This refactor was carried out over a series of iterative implementation rounds, each consisting of an AI code agent implementing changes based on a plan, followed by a human review that identified remaining issues, which were then documented and fed back into the next round.
...
2. **Round 1 review** (`issues.md` → `issues-resolution-plan.md`): After the initial implementation, 11 issues were identified spanning `conventions.py`, `circuit.py`, `parsed_lattice_result.py`, `run/functions.py`, and tests. These included: relaxing the Hamiltonian value flattening to preserve dict structure, extracting `trunc_string` from metadata instead of hard-coding it, restoring a removed d=2 ASCII diagram, removing a redundant `forder` argument from `LatticeStateEncoder`, adding a periodic boundary condition check in decoding, adding a `refresh` flag to `get_data_metadata`, fixing hard-coded control link indexing in `LatticeCircuitManager` to be F-order-aware, wiring `forder` through `ParsedLatticeResult` and the run module, and adding comprehensive tests for F-order validation and behavior. These were resolved in three phases (foundation fixes, F-order plumbing, tests).
...
4. **Round 3 review** (`issues-round-3.md` → `issues-round-3-resolution-plan.md`): A performance bottleneck was discovered in `ParsedLatticeResult` construction—`copy.deepcopy(encoder)` and `encoder.__repr__()` were being called for every instance, costing ~260ms each for d=2 T1 configurations (15,715 plaquette states). The fix replaced deep copies with shared references (safe since the encoder is never mutated), cached the repr string on the encoder, cached hash values, and added bit-string-level `ParsedLatticeResult` caching in `functions.py`. This reduced per-instance construction time from 260ms to 0.1ms.
