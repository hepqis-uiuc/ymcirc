...
# TODO implement electric trotter step. Also consider moving these into the lattice_tools package?
...
if __name__ == "__main__":
    ...
    n_trotter_steps_cases = [1, 2] # Make this a list that iterates from 1 to 3
    sim_times = np.linspace(0, 3.0, num=2) # set num to 20 for comparison with trailhead
...
for n_steps in n_trotter_steps_cases:
    ...
    extracted_data.plot(y = "vacuum_persistence_probability", label = f"$N_T = {n_steps}$", ax=plt.gca())
...
# result_file_path = Path(__file__).parent / Path("out.csv")
# with result_file_path.open('w') as data_file:
#     df_job_results.to_csv('out.csv')
# print("Done. Goodbye!")
